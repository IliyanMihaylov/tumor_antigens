                        ...
                        <div v-show="item.show">
                          ...
                          <v-list-item>
                            ...
                            <v-list-item-content class="align-end">
                              ...
                              <a
                                :href="'https://www.ncbi.nlm.nih.gov/protein/'+item.Entry"
                              >NCBI Link</a>
                            </v-list-item-content>
                          </v-list-item>
                          ...
                          <v-item-group>
                            <v-row>
                              <v-col
                                v-for="(citation, i) in item.PubMedID"
                                :key="i"
                                cols="12"
                                md="4"
                              >
                                <v-chip outlined>
                                  <a :href="citation">PubMed</a>
                                </v-chip>
                              </v-col>
                            </v-row>
                          </v-item-group>
...
<script>
  ...
  import deleteAntigens from './component/DeleteAntigens'
  ...
  export default {
    ...
    methods: {
      ...
      calculateSequence: function (seuqnce) {
        const sequenceItem = []
        const chunks = seuqnce.match(/.{1,10}/g)
        let index = -1
        ...
        axios
          ...
          .then(function (response) {
            for (let i = 0; i < response.data.length; i++) {
              ...
              const sequenceItem = []
              const chunks = currentData.Sequence.match(/.{1,10}/g)
              let index = -1
              for (let ch = 0; ch < chunks.length; ch++) {
                if (ch % 6 === 0) {
                  index++
